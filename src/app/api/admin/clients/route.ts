...
// Accepts PT display labels (UI) or DB enum values for status.
...
export async function POST(request: Request) {
  ...
  const status = parseStatus(body?.status) ?? "negotiating";
...
// Status changes (Ativo / Em negociação / Pausado) — owner and sales only.
export async function PATCH(request: Request) {
  if (!isSameOriginRequest(request)) return NextResponse.json({ error: "Origem inválida." }, { status: 403 });
  if (isRateLimited("admin-clients", getClientAddress(request), 60, 60_000)) return NextResponse.json({ error: "Muitas solicitações." }, { status: 429 });
  if (!hasJsonContentType(request)) return NextResponse.json({ error: "Formato inválido." }, { status: 415 });
