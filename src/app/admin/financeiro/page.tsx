...
export default async function FinanceiroPage() {
  ...
  return <div>
    ...
    <div className="admin-card mt-6">{paymentLinkData.error ? <p role="alert" className="rounded-lg border border-amber-300/20 bg-amber-300/[.05] p-3 text-xs text-amber-200">{paymentLinkData.error}</p> : <PaymentLinks initialLinks={paymentLinkData.links} clients={clients.map((client) => ({ id: client.id, name: client.name }))} deals={commercial.deals.map((deal) => ({ id: deal.id, name: deal.title }))} canManage={canManage}/>}</div>
