...
import {
  ...
  CircleDollarSign, LayoutDashboard, LogOut, Menu, Settings, Users, X,
} from "lucide-react";
    ...
    <div className="admin-shell min-h-screen bg-[#050510] text-[#e8e8f0]">
      <aside className={`admin-sidebar ${collapsed ? "admin-sidebar-collapsed" : ""} ${mobileOpen ? "admin-sidebar-mobile" : ""}`}>
        ...
        <div className="mt-auto p-3">
          ...
          <button className="admin-nav-link w-full"><Settings size={19} /><span>Configurações</span></button>
      ...
      <main className={`admin-main ${collapsed ? "admin-main-expanded" : ""}`}>
        <header className="admin-header">
          ...
          <div className="ml-auto flex items-center gap-3">
            <button aria-label="Notificações" className="admin-icon-btn relative"><Bell size={18} /><i className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-cyan-300" /></button>
