...
const STATUS_OPTIONS: { value: ClientStatus; api: string }[] = [
  ...
  { value: "Em negociação", api: "negotiating" },
  { value: "Pausado", api: "paused" },
];
...
export function CRMTable({ clients: initialClients, canCreate }: { clients: Client[]; canCreate: boolean }) {
  ...
  async function changeStatus(id: string, apiStatus: string) {
    const previous = clients;
    setClients((current) => current.map((c) => (c.id === id ? { ...c, status: STATUS_OPTIONS.find((s) => s.api === apiStatus)?.value ?? c.status } : c)));
    const response = await fetch("/api/admin/clients", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, status: apiStatus }) });
    if (!response.ok) {
      setClients(previous);
      setError("Não foi possível atualizar o status.");
      return;
    }
    router.refresh(); // keep overview KPIs and finance cards in sync
  }
